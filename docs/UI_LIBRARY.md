...
| `"use client"` — remove    | Remove when the wrapper is pure markup (no hooks, no handlers). Base UI's interactive dist files carry their own boundary, and `useRender` is server-safe; verify the specific dependency file before relying on this. | Unnecessary wrapper boundaries expand hydration ownership. See Section 7.                                                                           |
| `"use client"` — keep      | Keep when the wrapper itself calls hooks or attaches handlers. Current examples are AppShell, SiteShell, SkipLink, ThemeControl, and LocaleControl.                                                                    | Shells and controls own real runtime interaction, so these are narrow justified boundaries.                                                         |
...
- Registry vocabulary is kept as-is where it doesn't fight the identity (most controls `rounded-lg`, surfaces `rounded-xl`). Targeted departures: Button and Badge are `rounded-md` (Button matches the authored 6px control geometry; the registry Badge pill fights the flat radius scale), and the registry's translucent 3px focus halo (`ring-3 ring-ring/50`) is replaced everywhere by the designed focus language — a solid 2px `ring-ring` line, attached/offset/inset per control geometry (`docs/DESIGN_TOKENS.md` §2). Match the nearest existing primitive when in doubt.

## 7. Server Component Policy

**Wrappers stay Server Components whenever possible. Add `"use client"` only
when the wrapper's own implementation requires it.**

Base UI's interactive primitives carry client boundaries in their published
dist files, so a pure project wrapper can remain a Server Component. The
project library also has narrow first-party client boundaries where it owns
hooks or handlers: AppShell, SiteShell, SkipLink, ThemeControl, and
LocaleControl. Runtime providers have their own boundaries under
`src/core/providers`; `AppProvider` remains a Server Component that composes
them. When unsure, inspect both the wrapper and the installed Base UI dist file
instead of adding a directive defensively.
