...
The fonts are a separate matter, **unaffected** by the repo's own (absent) licence — their OFL obligations are owed to third parties. The built application self-hosts and **redistributes all three families** — Noto Sans Arabic, Public Sans, and Geist Mono — each under the SIL Open Font License 1.1. Noto is the only one committed as a source file (`src/assets/fonts/noto-sans-arabic-variable.woff2`), so it is the only one whose full OFL text is vendored in-repo (`src/assets/fonts/OFL.txt`) and must ship alongside it, regardless of this repo's public/private status. Public Sans and Geist Mono are fetched by `next/font` at build time and self-hosted into the build output (`.next/static/media/`); the generated woff2 files carry the copyright notice (name ID 0) and an OFL licence reference (name ID 14) in the font name-table metadata — the verbose licence text (name ID 13) is dropped by subsetting — which is how their notice obligation is met without an in-repo file.
