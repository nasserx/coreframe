...
export type FieldErrorProps = ComponentProps<"div"> & {
  /** Error objects to render; ignored when `children` is provided. */
...
/**
 * Form composition primitive: a structural wrapper that arranges a control
 * with FieldLabel, FieldContent, FieldDescription, and FieldError.
 * FieldGroup stacks fields, FieldSet/FieldLegend group related controls
 * with native semantics, and FieldSeparator divides sections. `orientation`
 * ("vertical" | "horizontal" | "responsive") controls layout.
 *
 * Accessibility: purely structural — consumers own the wiring: `htmlFor`/
 * `id` between FieldLabel and the control, `aria-describedby` pointing at
 * FieldDescription/FieldError ids, and `aria-invalid` on the control.
 * Invalid styling is driven by `data-invalid` on Field; FieldError renders
 * `role="alert"`.
 *
 * Constraints: composition only — no validation, no form state, no form
 * library coupling. FieldError accepts pre-produced messages (any source);
 * it never validates.
 */
