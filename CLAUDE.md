...
2. `src/styles/light.css` / `dark.css` — semantic `--color-*` and `--elevation-*` variables per theme, full parity required (`.dark` class toggles; `@custom-variant dark` in `theme.css`). Identity: flat/editorial — warm paper in light (oklch hue 84, cream cast) but a NEUTRAL near-black in dark (warmth reads as dirt in dark and was tinting light-on-dark text yellow — dark neutrals keep only trace chroma 0.003–0.008; light-on-dark foreground family sits at L 0.985 / chroma 0.002: near-white but deliberately short of pure white L 1.0, which halates/strains over long reading — see §2 "Dark foreground"), near-black primary (near-white in dark; no saturated hue on chrome — hue is reserved for status/charts), hairline borders carry structure, `--elevation-xs/sm` are empty (shadows exist only for floating layers md+). Dark surfaces separate by a lightness step (`background 0.185 → surface 0.225 → popover 0.26`) AND a visible hairline together — the step alone can't carry it near black. Border weight is fixed by the §2 **hairline rule** (quiet-but-present is a different contrast ratio per theme, since a light line on dark needs more than a dark line on light): light 0.9 (1.23:1 vs bg), dark 0.38 (1.86:1 vs bg); cards/popovers/dialogs draw `ring-border`. Control height is a four-step scale (§2): header action clusters use `lg` (h-9), utility toggles aligned up to match. Cross-theme values are deliberately asymmetric to achieve equal perception (§2 "Cross-theme perception"). See `docs/DESIGN_TOKENS.md` §2–3.
3. `src/styles/theme.css` — bridge mapping semantic variables into Tailwind v4 `@theme inline` and shadcn/ui variable names (`--primary`, `--card`, `--sidebar-*`, `--shadow-*`, …), plus the theme-neutral type ramp (`--text-display` … `--text-caption` → `text-*` utilities; display/title are 800-weight, tight-tracked, compact-leaded). No `--font-heading` — headings differ by weight/tracking only, one family (Geist) for headings and body.
