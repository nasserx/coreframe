...
1. `src/styles/base.css` — theme-neutral tokens (`--radius-base: 0.375rem`; controls sit at `rounded-lg` = 6px, surfaces at `xl`+ via the multiplier scale in `theme.css` — crisp controls, recognisably rounded surfaces; Badge is deliberately `rounded-md`, not the registry pill).
